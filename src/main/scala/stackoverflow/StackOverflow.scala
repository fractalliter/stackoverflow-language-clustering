...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(
    ...
    val questions =
      postings.filter(_.postingType == 1).map(post => (post.id, post))
    val answers =
  ...
  def scoredPostings(
    ...
    grouped
      .mapValues(it => (it.head._1, answerHighScore(it.map(_._2).toArray)))
      .values
